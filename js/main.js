...
// ─── QUERDLE LIVE DEMO ─────────────────────────────────
function initQpDemo() {
  var boardEl = document.getElementById('qp-board');
  if (!boardEl) return;
  ...
  function scoreGuess(guess, answer) {
    ...
    var a = answer.split(''), g = guess.split('');
    for (var i = 0; i < 5; i++) {
  ...
  var PLAYERS = [
    { name: 'xXDragon99Xx',  color: '#fb923c', badge: 'Wordsmith' },
    { name: 'speedrunqueen', color: '#2dd4bf', badge: null         },
    { name: 'nachtfalter_',  color: '#a78bfa', badge: 'Legend'     },
    { name: 'turtleshell42', color: '#fbbf24', badge: null         },
  ];
  var BONUS = { name: 'pixelwitch_88', color: '#f472b6' };

  var CHAT = [
    [  0,  -1, 'Round 47 started! Type !join to queue'],
    [700,   3, '!join'],
    [1000,  4, 'PogChamp PogChamp'],
    [1300,  0, '!join'],
    [1700,  1, '!join'],
    [2000,  0, '!guess STONE'],
    [2300, -1, 'xXDragon99Xx guessed STONE'],
    [3900,  1, 'E is in position 5!'],
    [4400,  3, 'no S T O or N tho'],
    [5200,  1, '!guess MALES'],
    [5500, -1, 'speedrunqueen guessed MALES'],
    [7000,  0, 'M A L E all yellow !!!'],
    [7500,  3, 'need F at the start!!'],
    [7900,  2, 'I got it'],
    [8500,  2, '!guess FLAME'],
    [8800, -1, 'nachtfalter_ guessed FLAME'],
    [10100, 1, 'YESSS FLAME PogChamp'],
    [10400, 3, 'monkaS LETS GOOOO'],
    [10700, 0, 'nachtfalter_ with the W!!'],
  ];

  var PRIORITY = { correct: 3, present: 2, absent: 1 };
  var KEY_ROWS = ['QWERTYUIOP', 'ASDFGHJKL', 'ZXCVBNM'];

  var playersEl, chatMsgsEl, queueCntEl, keyboardEl, captionEl, winBannerEl;

  function init() {
    boardEl     = document.getElementById('qp-board');
    playersEl   = document.getElementById('qp-players');
    chatMsgsEl  = document.getElementById('qp-chat-msgs');
    queueCntEl  = document.getElementById('qp-queue-cnt');
    keyboardEl  = document.getElementById('qp-keyboard');
    captionEl   = document.getElementById('qp-caption');
    winBannerEl = document.getElementById('qp-win-banner');
  }
  ...
  function buildBoard() {
    ...
    for (var r = 0; r < 6; r++) {
      ...
      for (var c = 0; c < 5; c++) {
        ...
        tile.className = 'qp-tile qp-tile--empty';
        tile.id = 'qt-' + r + '-' + c;
  ...
  function buildQueue(fromIdx) {
    playersEl.innerHTML = '';
    var remaining = PLAYERS.slice(fromIdx);
    queueCntEl.textContent = remaining.length;
    remaining.forEach(function(p, i) {
      var el = document.createElement('div');
      el.className = 'qp-player' + (i === 0 ? ' qp-player--active' : '');
      if (i === 0) {
        var dot = document.createElement('span');
        dot.className = 'qp-player__dot';
        el.appendChild(dot);
      }
      var posEl = document.createElement('span');
      posEl.className = 'qp-player__pos';
      posEl.textContent = i + 1;
      el.appendChild(posEl);
      var nameEl = document.createElement('span');
      nameEl.className = 'qp-player__name';
      nameEl.style.color = p.color;
      nameEl.textContent = p.name;
      el.appendChild(nameEl);
      if (p.badge) {
        var badgeEl = document.createElement('span');
        badgeEl.className = 'qp-player__badge';
  ...
  function addMsg(pIdx, text) {
    var el = document.createElement('div');
    if (pIdx === -1) {
      el.className = 'qp-msg qp-msg--system';
      el.textContent = text;
    } else {
      var p = pIdx === 4 ? BONUS : PLAYERS[pIdx];
      el.className = 'qp-msg';
      var u = document.createElement('span');
      u.className = 'qp-msg__user';
      u.style.color = p.color;
      u.textContent = p.name + ': ';
      var t = document.createElement('span');
      t.className = 'qp-msg__text';
      t.textContent = text;
      el.appendChild(u);
      el.appendChild(t);
    }
    chatMsgsEl.appendChild(el);
    while (chatMsgsEl.children.length > 7) chatMsgsEl.removeChild(chatMsgsEl.firstChild);
  }

  function fillRow(rowIdx, word) {
    word.split('').forEach(function(letter, c) {
      var tile = document.getElementById('qt-' + rowIdx + '-' + c);
      tile.textContent = letter;
      tile.style.animation = 'none';
      void tile.offsetWidth;
      tile.className = 'qp-tile qp-tile--pending';
    });
  }

  function revealTile(rowIdx, c, letter, result, delay, isLast, onDone) {
    later(function() {
      var tile = document.getElementById('qt-' + rowIdx + '-' + c);
      tile.style.animation = 'none';
      void tile.offsetWidth;
      tile.style.animation = 'qp-close 0.18s ease-in forwards';
      later(function() {
        tile.className = 'qp-tile qp-tile--' + result;
        tile.style.animation = 'none';
        void tile.offsetWidth;
        tile.style.animation = 'qp-open 0.18s ease-out both';
        var cur = keyState[letter];
        if ((PRIORITY[result] || 0) > (PRIORITY[cur] || 0)) {
          keyState[letter] = result;
          var keyEl = document.getElementById('qk-' + letter);
          if (keyEl) keyEl.className = 'qp-key qp-key--' + result;
        }
        if (isLast && onDone) later(onDone, 200);
      }, 185);
    }, delay);
  }

  function revealRow(rowIdx, word, results, onDone) {
    word.split('').forEach(function(letter, c) {
      revealTile(rowIdx, c, letter, results[c], c * 230, c === 4, onDone);
    });
  }

  function startDemo() {
    init();
    if (!boardEl || !document.contains(boardEl)) { clearAll(); return; }
    ...
    captionEl.textContent = 'Live demo · Suggestion Mode · word: ????';
    captionEl.style.color = '';
    winBannerEl.classList.remove('visible');

    CHAT.forEach(function(entry) {
      (function(d, p, t) {
        later(function() { addMsg(p, t); }, d);
      })(entry[0], entry[1], entry[2]);
    });

    later(function() {
      fillRow(0, 'STONE');
      var res = scoreGuess('STONE', WORD);
      later(function() { revealRow(0, 'STONE', res, null); }, 280);
    }, 2000);

    later(function() { buildQueue(1); }, 5000);
    later(function() {
      fillRow(1, 'MALES');
      var res = scoreGuess('MALES', WORD);
      later(function() { revealRow(1, 'MALES', res, null); }, 280);
    }, 5200);

    later(function() { buildQueue(2); }, 8300);
    later(function() {
      fillRow(2, 'FLAME');
      var res = scoreGuess('FLAME', WORD);
      later(function() {
        revealRow(2, 'FLAME', res, function() {
          winBannerEl.classList.add('visible');
          captionEl.textContent = 'FLAME solved in 3 guesses — word: FLAME';
          captionEl.style.color = '#22c55e';
        });
      }, 280);
    }, 8500);

    later(startDemo, 14000);
  }
